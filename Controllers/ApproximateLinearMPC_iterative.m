...
%}
classdef ApproximateLinearMPC_iterative < Controller
    ...
    properties (SetAccess = private)
        ...
        sdp_setting
        controller
        solvingIterationsCounter
    end
    
    
    methods
        function obj = ApproximateLinearMPC_iterative(zoneName, delayCurtailment, delayBattery, delayTelecom, ...
        ...
        function setOtherElements(obj, amplifierQ_ep1, maxPowerGeneration, ...
           ...
           if isObjective_Guillaume1_NoBattery
            ...
            obj.countControls = 0;
            obj.countSolvingIterations = 0;
            obj.solvingIterationsCounter = 0;
        end
    ...
    methods
        ...
        function operateOneOperation(obj, realState, realDeltaPA, realDeltaPT)
            ...
            currentOptimalNextDeltaPC = zeros(obj.c, 1);
            thresholdIteration = 0.1;

            % end the while loop

            obj.interpretResult();

            while sumabs(currentOptimalNextDeltaPC - obj.ucK_new) > thresholdIteration
                obj.countSolvingIterations = obj.countSolvingIterations + 1;
                currentOptimalNextDeltaPC = obj.ucK_new;

                % repeat the optimization by changing the expectedDeltaPC.
                obj.updateDelta_PC_est_over_horizon(currentOptimalNextDeltaPC);
                obj.setPC_est_over_horizon();
                obj.setDelta_PG_and_PG_est_over_horizon();
            ...
            obj.solvingIterationsCounter(end+1) = obj.countSolvingIterations;
            obj.saveInfeas();
            obj.saveEpsilon();
        ...
        function updateDelta_PC_est_over_horizon(obj, newNextDeltaPC)
            numberOfStepsAfterDelay = obj.N - obj.tau_c;
            noCurtControlAfterFollowing = zeros(obj.c, numberOfStepsAfterDelay -1);
            obj.Delta_PC_est = [obj.ucK_delay newNextDeltaPC noCurtControlAfterFollowing];
        end
        ...
        function set_xK_extend(obj, realState)
            realState.setPowerAvailable(obj.PA_est(:,1)); % useless? because the available power is not used, see the following lines
