...
%}
classdef MpcWithUncertainty < Controller
    ...
    methods
        ...
        function setMpcFormulation(obj)
            obj.constraints = [obj.constraints, (obj.epsilon >= 0):'epsilon'];
            for kk = 1:obj.Ns
                for k = 1:obj.N
                    %%% constraints valid for all the forecast horizon
                    obj.constraints = [obj.constraints, (obj.x(:,k+1+(kk-1)*(obj.N+1)) == obj.A_new*obj.x(:,k+(kk-1)*(obj.N+1)) + obj.B_new*obj.u(:,k) +...
                            obj.D_new_in*obj.dk_in(:,k+(kk-1)*obj.N) + obj.D_new_out*obj.dk_out(:,k)):['flow conservation, ' num2str(k)]];
                    obj.constraints = [obj.constraints, (obj.umin <= obj.u(:,k)):['lower bound control u, ' num2str(k)]];
                    obj.constraints = [obj.constraints, (obj.u(:,k)<= obj.umax):['upper bound control u, ' num2str(k)]];
                    
                    if obj.tau_b > obj.tau_c % which is never, but be prepared
                        % valid for all the forecast horizon
                        obj.objective = obj.objective + obj.x(:,k+1+(kk-1)*(obj.N+1))'*obj.Q*obj.x(:,k+1+(kk-1)*(obj.N+1)) + obj.u(:,k)'*obj.R*obj.u(:,k);
                        
                        if k >= obj.tau_c + 1
                            obj.constraints = [obj.constraints, obj.xmin(1:obj.numberOfBranches).*(1 + obj.epsilon(:,k,kk)) <= obj.x(1:obj.numberOfBranches,k+1+(kk-1)*(obj.N+1))...
                                <= obj.xmax(1:obj.numberOfBranches).*(1 + obj.epsilon(:,k,kk))]; % extending boundary conditions for power flows
                            obj.constraints = [obj.constraints, obj.xmin(obj.numberOfBranches+1:end) <= obj.x(obj.numberOfBranches+1:end,k+1+(kk-1)*(obj.N+1))...
                                <= obj.xmax(obj.numberOfBranches+1:end) ]; % rest of the boundary conditions for the state
                        end
                    else
                        obj.constraints = [ obj.constraints, (obj.xmin(1:obj.numberOfBranches).*(1 + obj.epsilon(:,k,kk)) <= obj.x(1:obj.numberOfBranches,k+1+(kk-1)*(obj.N+1))):['lower bound state x, ' num2str(k)] ];
                        obj.constraints = [ obj.constraints, (obj.x(1:obj.numberOfBranches,k+1+(kk-1)*(obj.N+1))<= obj.xmax(1:obj.numberOfBranches).*(1 + obj.epsilon(:,k,kk)) ):['upper bound state x, ' num2str(k)] ];
                        
                        obj.constraints = [ obj.constraints, ( obj.xmin(obj.numberOfBranches+1:end) <= obj.x(obj.numberOfBranches+1:end,k+1+(kk-1)*(obj.N+1)) ):['lower bound rest of state x, ' num2str(k)] ];
                        obj.constraints = [ obj.constraints, ( obj.x(obj.numberOfBranches+1:end,k+1+(kk-1)*(obj.N+1)) <= obj.xmax(obj.numberOfBranches+1:end) ):['upper bound rest of state x, ' num2str(k)] ];
                     
                        if k <= obj.tau_b
                            obj.objective = obj.objective + obj.x(:,k+1+(kk-1)*(obj.N+1))'*obj.Q*obj.x(:,k+1+(kk-1)*(obj.N+1)) +...
                            obj.u(:,k)'*blkdiag(10*eye(obj.c)*(2+1/(1+k^3)),0*(-0.9+k^3)*eye(obj.b))*obj.R*obj.u(:,k);
     
                            
                        elseif k <= obj.tau_c
                            obj.objective = obj.objective + obj.x(:,k+1+(kk-1)*(obj.N+1))'*obj.Q*obj.x(:,k+1+(kk-1)*(obj.N+1)) +...
                            obj.u(:,k)'*blkdiag(eye(obj.c)*(2+1/(1+k^3)),(-0.9+k^3)*eye(obj.b))*obj.R*obj.u(:,k);
                    ...
                    if obj.Ns ~= 1
                        obj.objective = obj.objective + obj.probs(kk,obj.N)*(obj.epsilon(:,k,kk)'*obj.Q_ep1*obj.epsilon(:,k,kk)); % adding the penalisation of the disturbance scenario
                    else
                        obj.objective = obj.objective + (obj.epsilon(:,k,kk)'*obj.Q_ep1*obj.epsilon(:,k,kk));
                    end
                end
                
                % capping the overshooting
                % each branch, each time step, each scenario
                if (obj.Ns ~= 1)
                    if (k<=obj.tau_c)
                       obj.constraints = [obj.constraints, obj.probs(kk,obj.N)*obj.epsilon(:,k,kk) <= obj.epsilon_max];
                    else
                       obj.constraints = [obj.constraints, obj.probs(kk,obj.N)*obj.epsilon(:,k,kk) <= 0];
                       obj.constraints = [obj.constraints, obj.probs(kk,obj.N)*obj.epsilon(:,k,kk) >= 0];
                    end

                else
                    if (k<=obj.tau_c)
                       obj.constraints = [obj.constraints, (obj.epsilon(:,k,kk) <= obj.epsilon_max):['upper bound epsilon, ' num2str(k)]];
                    else
                        obj.constraints = [ obj.constraints, (obj.epsilon(:,k,kk) == 0):['epsilon = 0, ' num2str(k)]];
                    end
                end
                
            end
            % redundant with the individual constraints above
            % constraints = [constraints, sum(epsilon(:,k,kk)*probs(kk,N)) <= Ns*N*nbranchNEW*epsilon_max]; 
